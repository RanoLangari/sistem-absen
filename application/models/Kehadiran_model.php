...
class Kehadiran_model extends CI_Model {
	...
	public function getAllKehadiranPerMonth()
	{
		$format = "Y-m";
	...
	public function getRekap()
	{
		$query = "SELECT DISTINCT presents.user_id, users.name AS name, positions.position_name AS position, 
				(SELECT COUNT(information) FROM presents WHERE (information = 'Hadir') AND (STATUS = 1) AND (presents.user_id = users.id_users)) AS Hadir,
				(SELECT COUNT(information) FROM presents WHERE (information = 'Ijin') AND (STATUS = 1) AND (presents.user_id = users.id_users)) AS Ijin,
				(SELECT COUNT(information) FROM presents WHERE (information = 'Sakit') AND (STATUS = 1) AND (presents.user_id = users.id_users)) AS Sakit,
