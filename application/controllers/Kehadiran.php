...
class Kehadiran extends CI_Controller
{
	...
	public function tabel_kehadiran()
	{
		...
		$data['absensi'] = $this->kehadiran->getAllKehadiranPerMonth();
	...
	public function cetak()
	{
		...
		$spreadsheet->getActiveSheet()
			->setCellValue('A1', "Rekap Absensi Laboratorium Bio Sciense ");
		...
		foreach ($data as $d) {
			$spreadsheet->getActiveSheet()
				...
				->setCellValue('D' . $row, $data[$indeks]['Hadir'])
				->setCellValue('E' . $row, $data[$indeks]['Sakit'])
				->setCellValue('F' . $row, $data[$indeks]['Ijin'])
